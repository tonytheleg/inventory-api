...
* Kafka commandline tools (available under `/opt/kafka/bin`)
...
```shell
source /usr/loca/bin/env-setup.sh
```

When leveraging any Kafka commandline tools, the `KAFKA_AUTH_CONFIG` can be provided to most commands using the `--command-config` flag to ensure authentication is used and avoid errors. Some commands have different names for this flag, see their respective `--help` commands for details.
